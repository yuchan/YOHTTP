...
- (void)viewDidLoad
{
    ...
    UIScrollView *sv = [[UIScrollView alloc] initWithFrame:CGRectMake(0.0, 0.0, self.view.frame.size.width, self.view.frame.size.height)];
    [self.view addSubview:sv];
    
    //This list is borrowed from SDWebImage.
    ...
    NSInteger i = 0;
    for (NSString *purl in objects) {
        UIImageView *imageV = [[UIImageView alloc] init];
        imageV.frame = CGRectMake(4.0 + (width + 4.0) * (i % 4), 4.0 + (height + 4.0) * (i/4), width, height);
        [imageV setImageWithUrl:purl complete:^(UIImage *image, NSString *url){
        }];
        [sv addSubview:imageV];
        i++;
    }
    
    [sv setContentSize:CGSizeMake(sv.frame.size.width, 4.0 + (height + 4.0) * ([objects count]/4 + 1))];
}
